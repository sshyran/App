...
#include "../include/minicpio.h"


typedef unsigned char byte;

int check_ramdisk_magic(char *magic, char *file)
{
    int count;
...
int uncpio_usage(char *prog)
{
    fprintf(stderr,"%s file.cpio [output directory [cpio.list]]\n", prog);
    return 1;
}
...
int main(int argc, char **argv)
{
    ...
    while(!feof(f)) {
        ...
        check_ramdisk_magic(header.magic, cpiofile);
